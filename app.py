...
# --- 1. دالة إنشاء Instance (تم تحديثها لتخطي خطأ 403) ---
def create_merchant_instance(phone):
    if not phone: return None, None
    
    # التعديل: استخدام النطاق المباشر بدون شرطة
    url = f"https://api.greenapi.com/partner/waInstance/create/{PARTNER_KEY}"
    
    headers = {
        "Content-Type": "application/json",
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)" # يمنع حظر nginx
    }
    
    # إرسال طلب POST مع بيانات الخطة
    payload = {
        "plan": "developer" 
    }
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            # إظهار السبب الحقيقي للخطأ (مهم جداً للتشخيص)
...
if not st.session_state.logged_in:
    ...
    # (أكواد التبويبات t1, t2, t3 تظل كما هي لضمان استقرار العمليات الحالية)
    with t1:
    ...
    with t4:
        ...
        if not m_id:
            st.info("💡 بصفتكِ مالكة النظام، يمكنكِ الآن تفعيل المتجر بضغطة واحدة.")
            if st.button("🚀 تفعيل خدمة الواتساب الآن"):
                with st.spinner("جاري إنشاء حساب تقني للمتجر..."):
                    res_id, _ = create_merchant_instance(st.session_state.merchant_phone)
                    if res_id:
                        st.success("✅ تم التفعيل! يمكنك الآن توليد الرمز.")
                        st.rerun()
        else:
