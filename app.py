...
# تعديل المنفذ ليتوافق مع السيرفر الحالي
...
if not st.session_state.logged_in:
    ...
    # --- تبويب ربط الواتساب (التصميم كما طلبتِ تماماً) ---
    with t4:
        ...
        if st.button("🔄 توليد رمز QR الجديد"):
            with st.spinner("جاري بدء الجلسة..."):
                try:
                    # طلب بدء الجلسة في المحرك
                    requests.post(f"{WPP_URL}/api/{session_id}/start-session", headers=headers)
                    st.session_state.show_qr = True
                    # إضافة تأخير بسيط ليتمكن المحرك من توليد الرمز قبل إعادة التشغيل
                    time.sleep(2) 
                    st.rerun()
                except Exception as e:
        ...
        if st.session_state.get('show_qr'):
            ...
            try:
                ...
                if qr_res.status_code == 200:
                    ...
                    st.warning("الرمز قيد التحضير... انتظر ثوانٍ وحدث الصفحة")
            except:
                st.error("تعذر جلب الرمز من السيرفر")
            
            # زر فحص الحالة (أضفته لكي لا يظل البرنامج في وضع الانتظار)
            if st.button("🔄 تحديث حالة الربط"):
                try:
                    ...
                    if status_res.get('status') is True:
                        ...
                        st.info("لم يتم المسح بعد، حاول مرة أخرى.")
                except:
                    st.error("خطأ في فحص الحالة")
